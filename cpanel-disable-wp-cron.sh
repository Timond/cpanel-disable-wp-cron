...
function ModifyCrontab() {
	...
	while read FILE_PATH; do
		if [ -n "$FILE_PATH" ]; then
			...
			echo "Adding */5 * * * * cd ""$FOLDER""; php -q wp-cron.php >/dev/null 2>&1 to "$CPANEL_USERNAME"'s crontab"
			( (crontab -l -u "$CPANEL_USERNAME" 2>/dev/null || echo "") ; echo "*/5 * * * * cd ""$FOLDER""; php -q wp-cron.php >/dev/null 2>&1") | sort -u | crontab -u "$CPANEL_USERNAME" -
		else
